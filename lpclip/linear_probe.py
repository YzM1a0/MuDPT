...
parser.add_argument("--trainval_dataset", type=str, default="", help="path to dataset")
parser.add_argument("--test_dataset", type=str, default="", help="path to dataset")
parser.add_argument("--num_step", type=int, default=8, help="number of steps")
parser.add_argument("--num_run", type=int, default=3, help="number of runs")
parser.add_argument("--feature_dir", type=str, default="clip_feat", help="feature dir path")
args = parser.parse_args()

trainval_dataset = args.trainval_dataset
trainval_dataset_path = os.path.join(f"{args.feature_dir}", trainval_dataset)

test_dataset = args.test_dataset
test_dataset_path = os.path.join(f"{args.feature_dir}", test_dataset)

print(f"loading train and val files of Dataset: {args.trainval_dataset}")
train_file = np.load(os.path.join(trainval_dataset_path, "train.npz"))
train_feature, train_label = train_file["feature_list"], train_file["label_list"]

val_file = np.load(os.path.join(trainval_dataset_path, "val.npz"))
val_feature, val_label = val_file["feature_list"], val_file["label_list"]

print(f"Loading test file of Dataset: {args.test_dataset}")
test_file = np.load(os.path.join(test_dataset_path, "test.npz"))
test_feature, test_label = test_file["feature_list"], test_file["label_list"]

os.makedirs(f"report/{test_dataset}", exist_ok=True)
val_shot_list = {1: 1, 2: 2, 4: 4, 8: 4, 16: 4}

for num_shot in [16, 8, 4, 2, 1]:
    ...
    for seed in range(1, args.num_run + 1):
        ...
        print(f"search c_weight accuracy: {acc_list}", flush=True)

        # binary search：二分查找c的范围
        peak_idx = np.argmax(acc_list)
        c_peak = search_list[peak_idx]
        c_left, c_right = 1e-1 * c_peak, 1e1 * c_peak

        def binary_search(c_left, c_right, seed, step, test_acc_step_list, test_macrof1_step_list, test_microf1_step_list):
            ...
            saveline = "{}, seed {}, {} shot, weight {}, test_acc {:.2f}\n".format(test_dataset, seed, num_shot, c_final, test_acc)
            with open("./report/{}/{}_s{}r{}_details.txt".format(test_dataset, args.feature_dir, args.num_step, args.num_run), "a+") as writer:
        ...
        for step in range(args.num_step):
            print(f"{test_dataset}, {num_shot} Shot, Round {step}: {c_left}/{c_right}", flush=True)
    ...
    save_line = "{}, {} Shot, Test acc stat: {:.2f} ({:.2f})\n".format(test_dataset, num_shot, acc_mean, acc_std)
    print(save_line, flush=True)
    with open("./report/{}/{}_s{}r{}.txt".format(test_dataset, args.feature_dir, args.num_step, args.num_run), "a+") as writer:
