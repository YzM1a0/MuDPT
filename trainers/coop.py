...
def load_clip_to_cpu(cfg):
    if not cfg.MODEL.BACKBONE.PATH:
        ...
        print(f"Loading backbone: {cfg.MODEL.BACKBONE.NAME} from {model_path}")
...
class PromptLearner(nn.Module):
    ...
    def construct_prompts(self, ctx, prefix, suffix, label=None):
        ...
        if self.class_token_position == "end":
            ...
            raise NotImplementedError(f"{self.class_token_position} is not implemented.")
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.tokenized_prompts = self.prompt_learner.tokenized_prompted
    ...
    def forward(self, image):
        image_features = self.image_encoder(image.type(self.dtype))  # (16, 3, 224, 224) -> (16, 1024)

        prompts = self.prompt_learner()  # (37, 77, 512)
        tokenized_prompts = self.tokenized_prompts  # (37, 77)
        text_features = self.text_encoder(prompts, tokenized_prompts)  # (37, 1024)

        image_features = image_features / image_features.norm(dim=-1, keepdim=True)
        text_features = text_features / text_features.norm(dim=-1, keepdim=True)

        logit_scale = self.logit_scale.exp()  # e^(self.logit_scale)
        logits = logit_scale * image_features @ text_features.t()  # (16, 37)
